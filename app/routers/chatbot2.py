...
# OLD: from app.services.love_brain import check_license  # module removed
from app.db.database import get_db
from app.schemas.chat import ChatRequest
import re
import difflib
# FAISS disabled — uncomment below to re-enable
# import numpy as np
# import faiss
...
from app.services.nl2sql_engine import (
    get_db_schema, generate_sql, format_answer, fuzzy_correct_query
)

# OLD: router = APIRouter(prefix="/chatbot", tags=["Chatbot"])
# NEW: changed to /v2-chatbot so Chat.jsx can connect to it
...
_ERP_KEYWORDS = {
    ...
    "mewar", "erp",
    "sale", "sales", "report", "employee", "account",
}
...
# FAISS disabled — startup is now instant
...
def smart_match(query_text, category="inventory"):
    # FAISS disabled — returns input as-is
    return query_text
# ==========================================
...
##### ==========================================
   ...
   # =========================================================
    ...
    # ── NL2SQL Fallback: if v2 gave only a clarification/suggestion message ─────
    if _is_poor_result(final_response):
        try:
            corrected_q  = fuzzy_correct_query(raw_q, db)
            schema_full  = get_db_schema(db, compact=False)
            schema_cmpct = get_db_schema(db, compact=True)
            sql          = generate_sql(corrected_q, schema_full, schema_cmpct)
            result       = db.execute(text(sql))
            columns      = list(result.keys())
            rows         = [list(row) for row in result.fetchall()]
            answer       = format_answer(corrected_q, rows, columns)
            nl2_parts    = [{"type": "chat", "message": answer}]
            if rows:
                nl2_parts.append({
                    "type":    "nl2sql_table",
                    "rows":    rows[:50],
            ...
            final_response = {"results": nl2_parts}
            print(f"[NL2SQL Fallback] answered: {raw_q!r}")
        except Exception as _e:
            print(f"[NL2SQL Fallback] failed: {_e}")
...
# ══════════════════════════════════════════════════════════════════════════════
# yaha se changes hai new ye endpoints exist nahi karte the pelhe so daal diye for buttons and list 
# ══════════════════════════════════════════════════════════════════════════════
# 
